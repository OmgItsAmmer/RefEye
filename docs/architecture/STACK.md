...
| Action spotting (M1 primary model) | **T-DEED** | Strong reported temporal precision for dense/tight action moments (shots, passes, contacts) on SoccerNet-style broadcast footage, and an actively maintained reference implementation — the best single bet for a one-model MVP. Wrapped entirely behind `ai/action_spotting/tdeed/TDeedActionSpotter` implementing the common `ActionSpotter` protocol (architecture §19) — no code outside this adapter may import T-DEED-specific types. FOOTPASS/TAAD and SoccerNet-oriented spotters remain as interface stubs (`ai/action_spotting/footpass`, `ai/action_spotting/soccernet`) for the Phase 2 benchmarking milestone. |
| Ball/player detection | YOLO-family detector (Ultralytics **YOLOv8/YOLO11**, small/nano variant) fine-tuned or zero-shot validated on sample footage, exported to ONNX | Fast enough to run continuously at low resolution; well-documented export path to ONNX Runtime. A generic "sports ball" class is *not* assumed sufficient (architecture §14) — validated against representative footage before being trusted, with confidence thresholds fully configurable. |
