...
#Local imports
from .modules import BaseRGBModel, EDSGPMIXERLayers, FCLayers, FC2Layers, step, process_prediction, process_double_head, process_labels
    ...
    class Impl(nn.Module):
        ...
        def __init__(self, args = None):
            ...
            if self._feature_arch.startswith(('rny002', 'rny008')):
                # pretrained=False (patched from upstream's True): the
                # checkpoint this app loads on top (adapter.py, strict=True)
                # overwrites every one of these weights anyway, so fetching
                # ImageNet init from Hugging Face Hub is a pure-waste network
                # dependency on first launch — and a hard failure on an
                # offline/restricted client machine, for weights that get
                # thrown away in the next few lines regardless.
